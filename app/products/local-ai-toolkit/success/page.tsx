...
export default function LocalAIToolkitSuccessPage() {
    ...
    if (sessionId) {
      fetch(`/api/verify-payment?session_id=${sessionId}`)
        ...
        .then((data) => {
          if (data.verified) {
            setStatus("verified");
            setDownloadUrl("/Local-AI-Toolkit.pdf");
          } else {
            setStatus("failed");
          }
        })
  ...
  return (
    <main className="min-h-screen bg-[#fbfbfb] py-16">
      <div className="max-w-2xl mx-auto px-4 text-center">
        ...
        <Link href="/products/local-ai-toolkit" className="inline-block text-blue-600 hover:underline">
          ← Try Again
        </Link>
      </div>
    </main>
